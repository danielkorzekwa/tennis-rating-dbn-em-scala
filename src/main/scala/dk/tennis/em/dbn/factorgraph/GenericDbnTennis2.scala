...
import Factor._
import dk.tennis.em.loopybp.ClusterGraph._
 ...
 *
 */
class GenericDbnTennis2(priorProb: Array[Double], emissionProb: Array[Double], transitionProb: Array[Double]) extends DbnTennis {
  ...
  val clusters: mutable.ListBuffer[Cluster] = mutable.ListBuffer()
  
  val edges:mutable.ListBuffer[Tuple2[Int,Int]] = mutable.ListBuffer()
  ...
  def getFactors(): List[Factor] = throw new UnsupportedOperationException("")
  ...
  private def addPlayerPriorFactor(playerName: String, timeSlice: Int) {
    ...
    val cluster = Cluster(playerVar.id,factor)
    clusters += cluster
  ...
  private def addPlayerTransitionFactor(playerName: String, maxPrevTimeSlice: Int, timeSlice: Int) {
    ...
    for (i <- maxPrevTimeSlice until timeSlice) {
      ...
      val factor = Factor(varCurr, varNext, transitionProb)
   
      clusters += Cluster(varNext.id,factor)
      
      edges += (varCurr.id ->varNext.id)
  ...
  private def addScoreFactor(result: Result) = {
    ...
    val scoreVarName = "score_" + result.playerA + "_" +  result.playerB +"_" + result.timeSlice + ":" + scoreIndex
    scoreIndex += 1
    ...
    if (result.playerAWinner.isDefined) {
      ...
      clusters += Cluster(scoreVarName.hashCode,emissionFactorWithEvidence)
    } else {
      clusters += Cluster(scoreVarName.hashCode,emissionFactor)
    }

    edges += (playerAVar.id -> scoreVar.id)
     edges += (playerBVar.id -> scoreVar.id)
    resultVariables += result -> scoreVar

  }

  private def createPlayerVariable(playerName: String, timeSlice: Int): Var = {
   val varName = playerName + "_rating_" + timeSlice
    Var(varName.hashCode, priorProb.size)
  }
